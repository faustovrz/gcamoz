...
printf '  2. SUPERSEDED -- this pipeline should NOT be used for polarity. Two faults:\n'
printf '     (a) every pileup site is ~DP=1, where bcftools call -m has no power and\n'
printf '         defaults to REF (fabricated a 2:1 ref/alt skew);\n'
printf '     (b) "matches B73" is NOT the ref/alt question. DArT calls SNPs within\n'
printf '         its OWN tag clusters, so its ref allele equals the B73 base at only\n'
printf '         ~76%% of markers (measured: qtl_test_pileup_coords.R).\n'
printf '     Use scripts/qtl/qtl_cml530_alleles_direct.R instead: it reads the\n'
printf '     assembly base at the tag SnpPosition and asks which DArT allele CML530\n'
printf '     carries, which is what polarity requires (93.3%% phase-consistent vs\n'
printf '     53.4%% here).\n'
